...
import { inngestTestEmail } from '@/emails/inngestTestEmail'
import { env } from '@/env'

export const sendInngestTestEmail = async (email: string) => {
  let data
  ...
  try {
    data = await resend.emails.send({
      ...
      to: email,
      reply_to,
      subject: 'Email Triggered by Inngest',
      react: React.createElement(inngestTestEmail, {
        email,
      }),
      headers: {
